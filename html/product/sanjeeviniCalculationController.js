...
export const sanjeeviniCalculation = async (req, res) => {
  try {
    const {
      api_key,
      user_uni_id,
    ...
    const sanjeevini = await Sanjeevini.findOne({
      where: { id: sanjeevini_id, status: 1 },
    });
    if (!sanjeevini || sanjeevini.price <= 0) {
      return res.json({ status: 0, msg: 'Invalid Sanjeevini' });
    }

    const useramount = await getTotalBalanceById(user_uni_id) || 0;

    const subtotal = sanjeevini.price;
    let offer_amount = 0;
    ...
    // ✅ Final Data
    const data = {
      sanjeevini_amount: parseFloat(subtotal.toFixed(2)),
      reference_id,
    ...
    console.error('Error in sanjeeviniCalculation:', error);
