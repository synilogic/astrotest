...
export const paidKundliCalculation = async (req, res) => {
  ...
  try {
    ...
    // ✅ Dynamic config key generation
    const configKey = `kundli_${order_for}_price`;
    const priceCheck = Number(await getConfig(configKey));
    ...
    if (offer_code) {
      ...
      if (offer) {
        if (
          curDate.isBefore(moment(offer.offer_validity_from)) ||
          curDate.isAfter(moment(offer.offer_validity_to))
        ) {
