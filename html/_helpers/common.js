...
import { Op, Sequelize, literal, QueryTypes, col } from "sequelize";
...
export async function getAstrologerData(filter = {}, isFirst = false, req = null) {
  try {
    ...
    const queryOptions = {
      ...
      include: [
        ...
        },
        {
          ...
          as: "prices",
          attributes: ["id", "price", "astrologer_uni_id", "type",
            "actual_price", "time_in_minutes", "currency", "trash", "created_at", "updated_at"],
          where: { trash: 0 },
          required: false
        },
        {
    ...
    const formattedResult = result.map(astrologer => {
      ...
      const nextOnlineTime = constants.next_online_time || '';

      // Inject next_online_time into prices
      // ... existing code ...
      if (Array.isArray(astrologerData.prices)) {
        astrologerData.prices = astrologerData.prices.filter(price => price.currency === 'INR').map(price => {
          // Filter live schedules based on price type
          let nextOnlineTime = null;
          if (Array.isArray(astrologerData.liveschedule)) {
            const filteredLiveSchedules = astrologerData.liveschedule.filter(schedule => {
          ...
          return {
            ...
            type: price.type,
            price: parseInt(parseFloat(price.price), 10),
            actual_price: parseInt(parseFloat(price.actual_price), 10),
            ...
            next_online_time: nextOnlineTime || 'NA'
          };
        });
      }
      // ... existing code ...
      const DEFAULT_IMG = `${req.protocol}://${req.get("host")}/assets/img/customer.png`;
      ...
      astrologerData.user.full_info = `${astrologerData.user.name} (${astrologerData.user.email}) {${astrologerData.user.phone}} [${astrologerData.astrologer_uni_id}] [${astrologerData.status || 'InActive'}]`;
      // Ensure user object is included
      astrologerData.user.updated_at = astrologerData.user.updated_at
        ? dayjs(astrologerData.user.updated_at).format("YYYY-MM-DD HH:mm:ss")
        : null;
      ...
      astrologerData.follower_count = Array.isArray(astrologerData.followers)
        ? astrologerData.followers.length
        : 0;
      ...
      delete astrologerData.followers;
...
export const getCustomerQueueList = async (user_uni_id) => {
  try {
    ...
    const rows = await CallHistory.findAll({
      include: [
        ...
        },
        {
          ...
          required: false,
          on: {
            '$call_history.astrologer_uni_id$': { [Op.col]: 'user.user_uni_id' }
          }
        },
        {
          model: Astrologer,
          ...
          required: false,
          on: {
            '$call_history.astrologer_uni_id$': { [Op.col]: 'astrologer.astrologer_uni_id' }
          }
        }
      ],
      where: {
      ...
      attributes: [
        ...
        [Sequelize.col('astrologer.astro_img'), 'astro_img'],
        [Sequelize.col('astrologer.is_virtual'), 'is_virtual'],
        // total_waiting_time subselect
        [Sequelize.literal(`IFNULL((
           SELECT SUM(chw.waiting_time)
           FROM call_history AS chw
           WHERE chw.astrologer_uni_id = astrologer.astrologer_uni_id
             AND chw.status IN ('queue','queue_request','request','in-progress')
           GROUP BY chw.astrologer_uni_id
         ), 0)`),
        ...
        // total_queue_count subselect
        [
          Sequelize.literal(`IFNULL((
           SELECT SUM(IF(chq.waiting_time > 0, 1, 0))
           FROM call_history AS chq
           WHERE chq.astrologer_uni_id = astrologer.astrologer_uni_id
             AND chq.status IN ('queue','queue_request','request','in-progress')
           GROUP BY chq.astrologer_uni_id
         ), 0)`), 'total_queue_count']
    ...
    const result = [];
    // 2) for each row, compute queue sequence numbers and waiting times
    ...
    for (const row of rows) {
      ...
      const seqSql = `
        ...
        FROM (
          SELECT
            @rownum := @rownum + 1 AS my_queue_number,
            @waiting_time := @waiting_time + chn.waiting_time AS my_waiting_time,
            chn.waiting_time,
            chn.uniqeid,
            IF(
              chn.status = 'in-progress',
              TIMESTAMPDIFF(SECOND, chn.call_start, '${now}'),
              0
            ) AS in_progress_waiting,
            chn.customer_uni_id,
            chn.astrologer_uni_id
          FROM call_history AS chn,
               (SELECT @rownum := 0, @waiting_time := 0) r
          WHERE chn.astrologer_uni_id = '${row.astrologer_uni_id}'
            AND chn.status IN ('queue','queue_request','request','in-progress')
          ORDER BY chn.created_at ASC
        ) a
        WHERE a.customer_uni_id = '${user_uni_id}'
          AND a.uniqeid = '${row.uniqeid}'
        LIMIT 1
      `;
      const sequence = await sequelize.query(seqSql, { type: QueryTypes.SELECT, plain: true }) || {};
      // defaults if no sequence found
      const my_queue_number = sequence.my_queue_number || 0;
      const my_waiting_time = sequence.my_waiting_time || 0;
      const in_progress_waiting = sequence.in_progress_waiting || 0;
      // attach to row
      row.my_queue_number = my_queue_number;
      row.my_waiting_time = my_waiting_time;
      row.in_progress_waiting = in_progress_waiting;
      // 3) adjust totals by subtracting the current user's own waiting_time if still in queue
      const ongoing = await CallHistory.findOne({
        where: {
          uniqeid: row.uniqeid,
          status: { [Op.in]: ['queue', 'queue_request', 'request', 'in-progress'] }
        },
      ...
      if (ongoing) {
        let total_wait = +row.total_waiting_time - ongoing.waiting_time;
        let my_wait = row.my_waiting_time - ongoing.waiting_time;
        if (total_wait < 0) total_wait = 0;
        if (my_wait < 0) my_wait = 0;
        row.total_waiting_time = total_wait.toString();
        row.my_waiting_time = my_wait;
      }
      // 4) format astro_img with your helper
      const imgPath = constants.astrologer_image_path;
      const imgDefaultPath = constants.default_astrologer_image_path;
      row.astro_img = ImageShow(imgPath, row.astro_img, 'icon', imgDefaultPath);
      // 5) determine if in-app voice call is available
      row.in_app_call = row.call_type === 'call' && +row.is_inapp_voice_call === 1 ? 1 : 0;
      // 6) restructure the response to nest intake data
      const restructuredRow = {
        ...
        astrologer_uni_id: row.astrologer_uni_id,
        astro_img: row.astro_img,
        is_virtual: row.is_virtual,
        total_waiting_time: row.total_waiting_time,
        total_queue_count: row.total_queue_count,
        my_queue_number: row.my_queue_number,
        my_waiting_time: row.my_waiting_time,
        in_progress_waiting: String(row.in_progress_waiting),
        in_app_call: row.in_app_call,
        intake: {}
      };
      // Dynamically extract all intake fields
      Object.keys(row).forEach(key => {
        if (key.startsWith('intake.')) {
          const fieldName = key.replace('intake.', '');
          restructuredRow.intake[fieldName] = row[key];
        }
      });
      result.push(restructuredRow);
    }
    return result;
  } catch (error) {
    console.error("Error in getCustomerQueueList:", error);
...
// export const getAstrologerQueueList = async (user_uni_id) =>  {
//   try {
//     let result = [];

//     const queueList = await CallHistory.findAll({
//       include: [
//         {
//           model: User,
//           as: 'user',
...
//       const value = queueList[i];
//       const imgPath = constants.customer_image_path;
//       const imgDefaultPath = constants.default_customer_image_path;
//       queueList[i].customer_img = ImageShow(imgPath, value.customer_img, 'icon', imgDefaultPath);
//     }
...
//       {
//         model: User,
//         as: "user", // This alias must match your association
//         required: false,
//       },
//     ],
//   });
...
export const getAstroDiscountedPrice = (
  astrologer_uni_id = "",
  price = 0,
  discount_percent = 0
) => {
  ...
  const numericDiscountPercent = parseFloat(discount_percent) || 0;

  // Validate inputs
  if (
    typeof numericDiscountPercent === "number" &&
    numericDiscountPercent > 0 &&
    numericDiscountPercent <= 100 && // Add upper limit check
    typeof numericPrice === "number" &&
    numericPrice > 0
  ) {
    // If discount is 100%, return 0
    if (numericDiscountPercent >= 100) {
      return 0;
    }

    // Calculate discounted price
    const discountMultiplier = 1 - (numericDiscountPercent / 100);
    const discountedPrice = numericPrice * discountMultiplier;
...
export const getBestDiscountForService = (discounts, serviceType) => {
  if (!discounts || discounts.length === 0) return null;

  // Filter discounts for the specific service type
  const serviceDiscounts = discounts.filter(discount => {
    if (serviceType === 'call') return true;
    if (serviceType === 'chat') return true;
    if (serviceType === 'video') return true;
    return false;
  });
  ...
  if (serviceDiscounts.length > 0) {
    return serviceDiscounts.reduce((best, current) =>
...
export async function transformPricesWithDiscounts(prices, user_uni_id, astrologer_uni_id) {
  const now = new Date();
  ...
  const isFirstTimeUser = user_uni_id
    ? await isFirstUser(user_uni_id, astrologer_uni_id)
    : false;
  ...
  return prices?.map(price => {
    ...
    let priceType = '';
    if (['call', 'internal_call'].includes(price.type)) priceType = 'call';
    else if (price.type === 'chat') priceType = 'chat';
    else if (['video', 'videocall'].includes(price.type)) priceType = 'video';
    ...
    if (discount && discount.discount_percent > 0) {
      discountedPrice = getAstroDiscountedPrice(astrologer_uni_id, originalPrice, discount.discount_percent);
      discountedActualPrice = getAstroDiscountedPrice(astrologer_uni_id, originalActualPrice, discount.discount_percent);
    ...
    // First-time user free minutes logic
    let firstCallFreeMinutes = "";
    if (!isFirstTimeUser && ['call', 'chat', 'video'].includes(price.type)) {
    ...
    return {
      ...
      actual_price: discountedActualPrice,
      time_in_minutes: price.time_in_minutes,
      currency: price.currency,
      trash: price.trash,
...
export const getAstroPriceDataType = async (
  ...
  try {
    ...
    // First call price start
    let first_call_offer = getConfig("first_call_offer");

    // Dual offer start
    const checkAstroOfferAfterLimit =
      await checkAstroOfferAfterLimitExceed(astrologer_id);
    if (
      checkAstroOfferAfterLimit &&
      !isNaN(checkAstroOfferAfterLimit) &&
      checkAstroOfferAfterLimit > 0
    ) {
      first_call_offer = checkAstroOfferAfterLimit;
    }
    // Dual offer end

    if (first_call_offer && first_call_offer == "2") {
      if (customer_uni_id) {
        const first_call_price = getConfig("first_call_price_per_min");
        if (first_call_price && first_call_price > 0) {
          const isFirstCall = await isFirstUser(customer_uni_id, astrologer_id);
          if (!isFirstCall) {
            const call_type = result?.type;
            if (low_price_offer_on().includes(call_type)) {
              result.price = first_call_price;
              is_offer_applied = 1;
            }
          }
        }
    ...
    if (is_offer_applied === 0) {
      const astroDiscount = await getAstroDiscount(astrologer_id);
      if (astroDiscount) {
        result.price = getAstroDiscountedPrice(
          astrologer_id,
          result.price,
          astroDiscount.discount_percent
        );
...
export function freeMinutesOfferOn() {
  ...
  if (getConfig("free_minutes_offer_on_voice_call") == 1) {
    result.push("call");
  }

  if (getConfig("free_minutes_offer_on_chat") == 1) {
    result.push("chat");
  }

  if (getConfig("free_minutes_offer_on_video_call") == 1) {
    result.push("video");
  }
...
export async function getTotalBalanceById(uni_id, type = "") {
  try {
    const totalCrResult = await Wallet.findOne({
      attributes: [[Sequelize.literal("SUM(amount / exchange_rate)"), "total"]],
      where: {
        user_uni_id: uni_id,
        main_type: "cr",
        status: 1,
    ...
    const totalCr = parseFloat(totalCrResult.total || 0);
    const totalDr = parseFloat(totalDrResult.total || 0);
    const netTotal = totalCr - totalDr;
    return Math.round(netTotal * 100) / 100;
  } catch (error) {
    console.error("Error in getTotalBalanceById:", error);
    return 0;
  }
}
...
export const remainingChatTime = async (uniqeid = '') => {
  ...
  if (!callHistory) return result;
  const call_status = callHistory.status;
  result.call_status = call_status;

  let remaining_time_in_second = 0;
  // const call_start = formatDateTime(callHistory.call_start);
  const call_start = callHistory.call_start;
  const now = Math.floor(Date.now() / 1000); // current time in seconds
  const startTime = call_start ? Math.floor(new Date(call_start).getTime() / 1000) : 0;

  const waiting_time = callHistory.waiting_time || 0;

  const calculateRemaining = () => {
    const time_difference = now - startTime;
    return waiting_time - time_difference;
  };
  if (callHistory.call_type === 'chat' && ['in-progress', 'request'].includes(call_status)) {
    remaining_time_in_second = call_status === 'in-progress' ? calculateRemaining() : waiting_time;

  } else if (callHistory.call_type === 'video' && ['in-progress', 'request'].includes(call_status)) {
    remaining_time_in_second = call_status === 'in-progress' ? calculateRemaining() : waiting_time;

  } else if (callHistory.call_type === 'call' && ['in-progress', 'request'].includes(call_status)) {
    const isInAppCall = callHistory.is_inapp_voice_call;
    if (isInAppCall === 1) {
      remaining_time_in_second = call_status === 'in-progress' ? calculateRemaining() : waiting_time;
    }
  }
  ...
  if (remaining_time_in_second > 0) {
    ...
    result.minutes = +(remaining_time_in_second / 60).toFixed(2);
  } else {
    result.remaining_time_in_second = 0;
    result.minutes = 0;
  }
  return result;
};
...
export async function getTotalBalanceGiftById(uni_id) {
...
export async function getTotalBalanceMainById(uni_id) {
  try {
    const totalCrResult = await Wallet.findOne({
      attributes: [[Sequelize.literal("SUM(amount / exchange_rate)"), "total"]],
      where: {
...
//           status: "queue",
//         },
//         include: ["user"], // Ensure association to User is defined
//         order: [["id", "ASC"]],
//       });
...
//           if (
//             !isFirstCall &&
//             history.offer_type === 3 &&
//             first_call_free_minutes > 0 &&
//             free_minutes_offer_on().includes(history.call_type)
//           ) {
//             isFreeCall = 1;
//           }

//           if (astroPrice.price <= user_wallet_amt || isFreeCall === 1) {
//             if (
...
//               return getRequest; // Exit after first valid match
//             }
//           } else {
...
//     return getRequest; // Will return {} if no match found
//   } catch (error) {
    ...
    if (runningCount === 0) {
      ...
      for (const history of queuedCalls) {
        ...
        if (astroPrice.price > 0) {
          ...
          // if (astroPrice.price <= user_wallet_amt || isFreeCall === 1)
          if (
            (user_wallet_amt >= 0 && astroPrice.price <= user_wallet_amt) || isFreeCall === 1
          ) {
            if (
              !astrologer.next_request_time ||
              (astrologer.next_request_time && astrologer.next_request_time !== current_datetime)
            ) {
              // Update astrologer's next_request_time
              await astrologer.update(
                { next_request_time: current_datetime },
                { where: { astrologer_uni_id } }
              );
...
//       let call_type = initial_call_type;
...
//               as: 'user',
//               required: false,
//               where: Sequelize.literal('call_history.customer_uni_id = user.user_uni_id')
//           }]
//       });

//       if (calls) {
//           if (calls.call_type) call_type = calls.call_type;
...
//           let is_schedule_call_history = 0;
//           let is_schedule_chat = 0;

//           if (!calls.ref_id && calls.ref_id !== null) {
//               const approvedArchitectRequest = await getApprovedArchitectRequest(calls.ref_id);
//               if (!approvedArchitectRequest) {
//                   is_architect_call_history = 1;
//                   if (calls.call_type && calls.call_type === 'chat') is_architect_chat = 1;
//               }
//               console.log('::::aagye yha 1aagye:::');
//               const inProgressSchedule = await getInProgressSchedule(calls.ref_id);
//               if (inProgressSchedule) {
//                   is_schedule_call_history = 1;
//                   if (calls.call_type && calls.call_type === 'chat') is_schedule_chat = 1;
//               }
//           }
//           console.log('::::aagye2 yha aagye:::');

//           const offer_type = calls.offer_type && !isNaN(calls.offer_type) ? parseInt(calls.offer_type) : 0;
//           let isFreeFirstCall = 0;
//           let useAmount = 0;
//           let wallet_balance = 0;
//           let deductable_duration = duration;

//           if (is_architect_call_history === 0 && is_schedule_call_history === 0) {
//               const astroPrices = await getAstroPriceDataType(astrologer_uni_id, call_type, currency, calls.customer_uni_id);
//               wallet_balance = await getTotalBalanceById(user_uni_id);

//               const astroCharge = calls.charge ?? astroPrices.price;
//               const astroChargeMinutes = calls.charge_minutes ?? astroPrices.time_in_minutes;
...
//                   if (getConfig('free_duration') && getConfig('free_duration_allow') < duration) {
//                       deductable_duration = duration - getConfig('free_duration');
//                       if (deductable_duration < 0) deductable_duration = 0;
//                   }

//                   if (deductable_duration && getConfig('amount_deduct_minute_wise')) {
//                       const minutes = deductMinuteCalculat(astroChargeMinutes, deductable_duration);
//                       useAmount = Math.round(astroCharge * minutes * 100) / 100;
//                   } else if (deductable_duration) {
//                       useAmount = Math.round((astroCharge * (deductable_duration / 60)) * 100) / 100;
//                   }
...
//                   if (duration > 0 && deductable_duration > 0 && useAmount === 0 && astroCharge === 0) {
//                       deductable_duration = 0;
//                   }
//               }
//           }
//           console.log('::::aagye3 yha aagye:::');

//           const refund_call_days = getConfig('refund_call_days');
//           const refundValidDate = refund_call_days > 0 ? new Date(Date.now() + refund_call_days * 86400000) : '';
...
//           };
//           console.log('::::aagye4 yha aagye:::');
// console.log("ddadasdads:::",wallet_balance , useAmount , wallet_balance , useAmount);

//           if (wallet_balance > 0 && useAmount > 0 && wallet_balance >= useAmount) {
//               await CallHistory.update(callHistoryData, { where: { uniqeid } });

//               await removeBusyStatus(astrologer_uni_id);
//               console.log('::::aagye5 yha aagye:::');

//               ////
//               const admin_percentage = astroDetail?.admin_percentage > 0
//                   ? astroDetail.admin_percentage
//                   : getConfig('admin_percentage');

//               // let admin_percentage;
//               // if (
//               //     astroDetail.admin_percentage  &&
//               //     parseInt(astroDetail.admin_percentage, 10) > 0
//               // ) {
//               //     admin_percentage = astroDetail.admin_percentage;
//               // } else {
//               //     admin_percentage = getConfig('admin_percentage');
//               // }
//               ////
...
//               if (isFreeFirstCall === 1 && getConfig('fixed_amount_to_astrologer_after_limit_exceed') === 1) {

//                   const limitAllow = await checkAstroOfferCallLimitAllow(astrologer_uni_id);
//                   if (!limitAllow) {
//                       const fixedAmount = getConfig('astrologer_get_amount_after_limit_exceed');
...
//                               user_uni_id,
//                               call_type,
//                               useAmount: fixedAmount,
//                               duration,
//                               uniqeid,
//                               astro_offer_call_amount: 1
//                           };
//                           await walletHistoryCreate(postAstroWalletHistoryData);
//                       }
//                   }
...
//           }
//       } else {
//           if (history) await history.update({ status: 'Invalid Call.' });
//           await removeBusyStatus(astrologer_uni_id);
//           result = {
//               status: 0,
//               msg: "Invalid Call."
//           };
//       }
//       console.log('::::aagye6 yha aagye:::');

//       if (astrologer_uni_id) {
//           const waitingCustomer = await waitingCustomer(astrologer_uni_id);
//           if (waitingCustomer?.call_type) {
//               await startCall(waitingCustomer, waitingCustomer.call_type);
//           }
//       }
...
//       return {
//           status: 0,
//           msg: "An error occurred while processing the call transaction."
//       };
//   }
// };
...
export const callTransations = async (data) => {
  try {
    ...
    if (calls) {
      ...
      let offer_type = Number(calls.offer_type) || 0;
      let offer_type_txt = "";
      ...
      if (is_architect_call_history === 0) {
        ...
        if (isFreeFirstCall === 0) {
          if (
            getConfig("free_duration") &&
            getConfig("free_duration_allow") < duration
          ) {
            deductable_duration = duration - getConfig("free_duration");
            if (deductable_duration < 0) deductable_duration = 0;
          }
          ...
          if (deductable_duration && getConfig("amount_deduct_minute_wise")) {
            ...
            useAmount =
              Math.round(((astroCharge * deductable_duration) / 60) * 100) /
              100;
          }
      ...
      if (wallet_balance > 0 && useAmount > 0 && wallet_balance >= useAmount) {
        ...
        const admin_percentage =
          astroDetail?.admin_percentage || getConfig("admin_percentage");
        ...
        if (
          ...
          const offerLimitAllowed =
            await checkAstroOfferCallLimitAllow(astrologer_uni_id);
          if (!offerLimitAllowed) {
            const astro_amount =
              getConfig("astrologer_get_amount_after_limit_exceed");
            if (astro_amount > 0) {
              const postAstroWalletHistoryData = {
                astrologer_uni_id,
                admin_percentage: 0,
      ...
      if (astrologer_uni_id) {
        ...
        if (waiting?.call_type) {
          await startCall(waiting, waiting.call_type);
        }
      }
...
export const free_minutes_offer_on = () => {
  const result = [];
  if (getConfig("free_minutes_offer_on_voice_call") === '1') result.push("call");
  if (getConfig("free_minutes_offer_on_chat") === '1') result.push("chat");
  if (getConfig("free_minutes_offer_on_video_call") === '1') result.push("video");
  return result;
};
...
export const availableMinuteCalculat = async (
  ...
  try {
    ...
    if (uniqeid) {
      ...
      if (callHistory && callHistory.waiting_time) {
        const waiting_time = callHistory.waiting_time;
        total_minutes = waiting_time / 60;
      }
    }

    if (total_minutes <= 0) {
      const isFirstCall = await isFirstUser(customer_uni_id, astrologer_uni_id);
      let first_call_offer = getConfig("first_call_offer");

      const checkOffer = await checkAstroOfferAfterLimitExceed(astrologer_uni_id);
      ...
      if (!isFirstCall && first_call_offer) {
        if (first_call_offer === '2') {
          if (low_price_offer_on().includes(call_type)) {
            const low_price_offer_maximum_duration = getConfig("low_price_offer_maximum_duration");
            if (
              low_price_offer_maximum_duration &&
              !isNaN(low_price_offer_maximum_duration) &&
              low_price_offer_maximum_duration > 0
            ) {
              let total_minute_by_balance = 0;
              per_minutes = per_minutes || 1;
              if (price > 0 && per_minutes > 0 && total_balance > 0) {
                total_minute_by_balance = Math.floor(total_balance / price) * per_minutes;
              }

              if (total_minute_by_balance >= low_price_offer_maximum_duration) {
                total_minutes = low_price_offer_maximum_duration;
              } else {
                total_minutes = total_minute_by_balance;
              }
            }
          }
        } else if (first_call_offer === '3') {
          const check_astro_free_offer_limit = await checkAstroOfferCallLimitAllow(astrologer_uni_id);

          if (
            first_call_free_minutes &&
            first_call_free_minutes > 0 &&
            (check_astro_free_offer_limit === true || getConfig("fixed_amount_to_astrologer_after_limit_exceed") === 1)
          ) {
    ...
    if (total_minutes <= 0) {
      per_minutes = per_minutes || 1;
      if (price > 0 && per_minutes > 0 && total_balance > 0) {
        total_minutes = Math.floor(total_balance / price) * per_minutes;
      } else if (price === 0) {
        total_minutes = getConfig("full_discounted_call_max_minutes") || 15;
      }
    }
    ...
    const call_max_minutes = constants.call_max_minutes;
    if (call_max_minutes && !isNaN(call_max_minutes)) {
      const max = Math.floor(call_max_minutes);
    ...
    console.error('availableMinuteCalculat error:', error);
    return 0;
  }
};
...
export const exotelCustomerWhitelistCurl = async (Number) => {
  try {
    const postData = new URLSearchParams({
      VirtualNumber: getConfig('exotel_caller_id'),
      Number,
      Language: "en",
    });
...
export const exotelIncomingCallRequestCurl = async (PhoneNumber, call_type = '', in_app_voice_call = 0) => {
  try {
    const api_key = getConfig('export_api_key');
    const api_token = getConfig('export_api_token');
    const subdomain = getConfig('export_api_subdomaim');
    const exotel_sid = getConfig('export_sid');
    let ivr_app_id = getConfig('exotel_ivr_app_id');

    if (call_type === 'video') {
      ivr_app_id = getConfig('exotel_ivr_app_id_for_video_call');
    } else if (call_type === 'call') {
      if (in_app_voice_call && in_app_voice_call === 1) {
        ivr_app_id = getConfig('exotel_ivr_app_id_for_in_app_voice_call');
      }
    } else if (call_type === 'architect_service_order') {
      ivr_app_id = getConfig('exotel_ivr_app_id_for_architect_service_order');
    }
    ...
    if (ivr_app_id) {
      const postData = new URLSearchParams({
        ...
        Url: `http://my.exotel.com/${exotel_sid}/exoml/start_voice/${ivr_app_id}`,
      });

      // Replace <subdomain> with the region of your account
      // <subdomain> of Singapore cluster is @api.exotel.com
      // <subdomain> of Mumbai cluster is @api.in.exotel.com
      const url = `https://${api_key}:${api_token}${subdomain}/v1/Accounts/${exotel_sid}/Calls/connect.json`;
    ...
    console.error(
      "Exotel incoming call request error:",
      error.response?.data || error.message
    );
    return null;
  }
...
function newPushNotificationId(code) {
...
export const sendNotification = async (data) => {
  ...
  const rawData = {
    ...
    notification_id: data.notification_id ?? newPushNotificationId('PNOTI'),
  ...
  // Convert all values to strings to satisfy FCM's requirements
  const notificationData = Object.fromEntries(
    Object.entries(rawData).map(([key, val]) => [key, val?.toString?.() ?? ''])
  );

  data.chunk = Array.from(new Set(data.chunk || []));
  const tokens = data.chunk.filter(Boolean);

  if (!tokens.length) return { error: 'No valid tokens provided.' };
  ...
  try {
    const responses = await Promise.all(tokens.map(token =>
      messaging.send({ ...androidConfig, token })
    ...
    for (const res of responses) {
      if (res.success) {
        ...
        failed++;
        failedTokens.push(res.token);
        failureDetails.push({ token: res.token, error: res.error });
      }
    }

    const successfulTokens = tokens.filter(t => !failedTokens.includes(t));

    if (failureDetails.length === 0 && tokens.length === 1) {
      await User.update(
        { is_uninstalled: 0 },
        { where: { user_fcm_token: tokens[0] } }
      );
    }
    return {
      success,
...
export const minutesToReadableTime = (minutes) => {
...
// Exported function
export const firebaseCustomAuthToken = async (uid = '') => {
  if (!uid) return '';

  try {
    // Load service account JSON from _helpers/firebase/service-account.json
    const serviceAccountFile = JSON.parse(
...
//             const call_end = new Date(new Date(callHistory.call_start).getTime() + duration * 1000);
//             const sendData = {
...
//                 // const waiting_for_request = formatDateTime(new Date(Date.now() + request_waiting * 1000));
//                 const waiting_for_request = new Date(Date.now() + request_waiting * 1000);
...
//                           waiting_for_request: waiting_for_request,
//                           ivr_count: ivr_count,
//                           // ivr_start_from: formatDateTime(new Date(Date.now() + ivr_after_seconds * 1000)),
//                           ivr_start_from: new Date(Date.now() + ivr_after_seconds * 1000),
//                           };
//                         if (
//                           in_app_voice_call == 1 &&
//                           senddata.token &&
...
//                          // call_start: moment().format('YYYY-MM-DD HH:mm:ss'),
//                         // order_date: moment().format('YYYY-MM-DD'),
//                     call_start: new Date().toISOString(),
//                         order_date: new Date().toISOString().slice(0, 10),
//                         status: "in-progress",
//                       };
//                       console.log("virtualReqUpdateData:::",virtualReqUpdateData);
          ...
          if (astro_online_status == 1) {
            if (
              ...
              if (is_queue_request) {
                ...
                if (!isFirstCall) {
                  let first_call_offer = getConfig("first_call_offer");
                  const checkAstroOfferAfterLimitExceeds =
                    await checkAstroOfferAfterLimitExceed(
                      array.astrologer_uni_id
                    );
                  if (
                    checkAstroOfferAfterLimitExceeds &&
                    Number.isInteger(checkAstroOfferAfterLimitExceeds) &&
                    checkAstroOfferAfterLimitExceeds > 0
                  ) {
                    first_call_offer = checkAstroOfferAfterLimitExceeds;
                  }
    ...
    if (res && res.length) {
      ...
      for (const record of res) {
        ...
        if (custImg && fs.existsSync(path.join(imgDir, custImg))) {
          record.dataValues.customer_img = `${filterArray.basePath}/${constants.customer_image_path}${custImg}`;
        } else {
          record.dataValues.customer_img = `${filterArray.basePath}/${defaultImg}`;
        }
...
export const uploadOtherFiles = async (fileObj, filePath, fileType = "file") => {
  try {
    if (!fileObj || !fileObj.originalname || !fileObj.path) {
      return false;
    }
    ...
    // Move file from multer temp path to destination
    fs.renameSync(fileObj.path, finalPath);

    return filename;
  } catch (err) {
    console.error("uploadOtherFiles error:", err);
    return false;
  }
};
...
export const uploadImage = async (file, imgPath, imgKey, lastFile = "") => {
  try {
    ...
    const ext = path.extname(file.originalname); // <-- fix here
    ...
    // Move original file
    await fs.promises.rename(file.path, originalPath); // <-- fix here
...
//   try {
//     const basePath = attributes.basePath;
...
//     let is_virtual_astrologer_first_msg =
//       attributes.is_virtual_astrologer_first_msg || 0;
...
//     let file_url =  `${basePath}${imagePath.chat_file_path}${ attributes.file_url}` || "";
...
//     attributes.file_url =attributes.message_type == 'Text'? "": file_url  
...
//           file_url:
//             create.file_url ||"",
//           call_type: create.call_type,
//           is_assistant_chat: create.is_assistant_chat || 0,
//           status: create.status,
//           updated_at: formatDateTime(create.updated_at),
//           created_at: formatDateTime(create.created_at),
//           id: create.id,
//         },
//         msg: "create data",
//       };
...
//       let astrologer_img = `${basePath}${constants.default_astrologer_image_path}`;
...
//             let imgPath = basePath + constants.astrologer_image_path;

//             if (astro_img && fs.existsSync(path.join(imgPath, astro_img))) {
//               astrologer_img = `${basePath}/${constants.astrologer_image_path}${astro_img}`;
//             }
//           }
//           //old
...
//           //     let imgPath = basePath + (constants.astrologer_image_path);
//           //     if (astro_img && fs.existsSync(path.join(imgPath, astro_img))) {
//           //       astro_img = `${basePath}${constants.astrologer_image_path}${astro_img}`;
//           //     } else {
//           //       astro_img = `${basePath}${constants.default_astrologer_image_path}`;
//           //     }
...
//           // }
//           if (is_first_chat === 0) {
//             const customer = await getCustomerById(parts[0]);
//             const notification_id = uniqeid.replace(/(CALL|CHAT|VIDEO)/g, "");
//             let message_type = create.message_type;
//             let notif_message = create.message;
//             let notifi_title = "";
//             let notifi_image = "";
...
//               message_type === "ManualServices"
//             ) {
//               const decoded = notif_message ? JSON.parse(notif_message) : "";
//               notifi_title = " suggested you a Remedy";
//               notif_message = "Suggested a remedy for you.";

//               if (decoded) {
//                 deeplink_url = decoded.productUrl || "";
//                 const productName = decoded.name || "";
//                 notif_message = `${productName} is suggested to you by ${astrologer_name}, avail this now.`;
//                 notifi_image = file_url;
//               }
//             } else {
...
//             ) {
//               let astro_img = astrologer.astro_img;
//               const imgPath = basePath + constants.astrologer_image_path;
...
//               sendNotification(customerNotify);
//             } else if (customer.name && astrologer.user_fcm_token) {
//               const astroNotify = {
//                 title: customer.name,
//                 description: notif_message,
//                 chunk: [astrologer.user_fcm_token],
...
//         if (open_ai_message) {
//           if (realtime_database && realtime_database !== false) {
...
export const first_call_free_minutes_text = async (minutes) => {
  const prefix = getConfig("first_call_free_minute_prefix");
  const suffix = getConfig("first_call_free_minute_suffix");
...
export const checkCallDetail = async (
  ...
  try {
    ...
    if (customer) {
      ...
      const astrodataRaw = await Astrologer.findOne({ //errr
        where: { astrologer_uni_id },
        subQuery: false,
        include: [
          {
        ...
        attributes: [
          ...
          [
            Sequelize.fn(
              "IFNULL",
              Sequelize.fn("MAX", Sequelize.col("call_history.waiting_time")), // Use MAX to aggregate
              0
            ),
            "total_waiting_time",
          ],
        ...
        group: [
          ...
          "busy_status",
          Sequelize.literal("astrologers.id"),
        ],
      });
      ...
      if (astrodata && astrodata.price > 0) {
          ...
          if (!isFirstCall && first_call_offer) {
            ...
            if (
              ...
              if (
                ...
                first_call_free_minutes_txt = await first_call_free_minutes_text(
                  first_call_free_minutes
                );
                actual_price = astrodata.price;
                is_offer_applied = 1;
        ...
        if (is_offer_applied === 0) {
          ...
          if (astroDiscount) {
            actual_price = astrodata.price;
            astrodata.price = getAstroDiscountedPrice(
              astrologer_uni_id,
              astrodata.price,
              astroDiscount.discount_percent
            );
...
// export const getChatChannelHistory = async (request) => {
//   try {
//     // Get base URL from request or use constants as fallback
//     const baseUrl = request.baseUrl;

//     const whereClause = { call_type: "chat" };

//     if (request.first_msg_id) {
//       whereClause.id = { [Op.lte]: request.first_msg_id };
//     }

//     if (request.is_assistant_chat === 0 || request.is_assistant_chat === 1) {
//       whereClause.is_assistant_chat = request.is_assistant_chat;
//     }

//     if (request.channel_name) {
//       whereClause.channel_name = request.channel_name;
//     }

//     if (request.user_uni_id && request.user_uni_id.includes("CUS")) {
//       whereClause.trash = 0;
//     }

//     const page = request.page || 1;
//     const page_limit = constants.chat_api_page_limit

//     const offset = (page - 1) * page_limit;

//     const chat_channel_history = await ChatChannelHistory.findAll({
//       include: [
//         {
//           model: User,
//           as: "user",
...
//           required: false,
//           on: {
//             astrologer_uni_id: Sequelize.where(
//               Sequelize.col("user.user_uni_id"),
//               "=",
//               Sequelize.col("astrologer.astrologer_uni_id")
//             ),
//           },
//         },
//         {
...
//           required: false,
//           on: {
//             customer_uni_id: Sequelize.where(
//               Sequelize.col("user.user_uni_id"),
//               "=",
//               Sequelize.col("customer.customer_uni_id")
//             ),
//           },
//         },
//       ],
...
//         [
//           Sequelize.literal(
//             `IF(ChatChannelHistory.user_uni_id LIKE '%ASTRO%', astrologer.display_name, user.name)`
//           ),
//           "user_name",
//         ],
//       ],
...
//     const chat_channels = chat_channel_history.map((value) => {
//       // const display_name = value.display_name || '';
//       const user_name = value.user_name || "";
//       const customer_img_path =
//         (constants.customer_image_path, value.customer_img || "");
//       const astrologer_img_path =
//         (constants.astrologer_image_path, value.astro_img || "");

//       const customer_img =
//         value.customer_img && fs.existsSync(customer_img_path)
//           ? `${baseUrl}${constants.customer_image_path}${value.customer_img}`
//           : `${baseUrl}${constants.default_customer_image_path}`;

//       const astro_img =
//         value.astro_img && fs.existsSync(astrologer_img_path)
//           ? `${baseUrl}${constants.astrologer_image_path}${value.astro_img}`
//           : `${baseUrl}${constants.default_astrologer_image_path}`;

//       return {
//         ...value,
//         // display_name,
//         user_name,
//         user_image_url: value.user_uni_id.includes("ASTRO")
//           ? astro_img
//           : customer_img,
...
//   } catch (error) {
//     console.error("Error in getChatChannelHistory:", error);
//     throw error;
//   }
// };
...
export const getChatChannelHistory = async (request) => {
  try {
    ...
    // First, get the chat messages without joins to avoid duplicates
    const chat_channel_history = await ChatChannelHistory.findAll({
      where: whereClause,
      order: [["id", "DESC"]],
      offset,
    ...
    // Then, enrich each message with user information
    const chat_channels = await Promise.all(
      chat_channel_history.map(async (value) => {
        let user_name = "";
        let customer_img = "";
        let astro_img = "";

        // Determine user type and get appropriate data
        if (value.user_uni_id.includes("ASTRO")) {
          // Get astrologer data
          const astrologer = await Astrologer.findOne({
            where: { astrologer_uni_id: value.user_uni_id },
            attributes: ["display_name", "astro_img"],
            raw: true,
          });
          
          user_name = astrologer?.display_name || "";
          const astrologer_img_path = constants.astrologer_image_path + (astrologer?.astro_img || "");
          
          astro_img = astrologer?.astro_img && fs.existsSync(astrologer_img_path)
            ? `${baseUrl}${constants.astrologer_image_path}${astrologer.astro_img}`
            : `${baseUrl}${constants.default_astrologer_image_path}`;
        } else {
          // Get user data (name is in users table)
          const user = await User.findOne({
            where: { user_uni_id: value.user_uni_id },
            attributes: ["name"],
            raw: true,
          });
          
          // Get customer image data
          const customer = await Customer.findOne({
            where: { customer_uni_id: value.user_uni_id },
            attributes: ["customer_img"],
            raw: true,
          });
          
          user_name = user?.name || "";
          const customer_img_path = constants.customer_image_path + (customer?.customer_img || "");
          
          customer_img = customer?.customer_img && fs.existsSync(customer_img_path)
            ? `${baseUrl}${constants.customer_image_path}${customer.customer_img}`
            : `${baseUrl}${constants.default_customer_image_path}`;
        }
        ...
        return {
          ...value,
          user_name,
          user_image_url: value.user_uni_id.includes("ASTRO") ? astro_img : customer_img,
          parent_id: value.parent_id || "",
          selected_text: value.selected_text || "",
          created_at: formatDateTime(value.created_at),
          updated_at: formatDateTime(value.updated_at),
...
export const inProgressIntakeDetailForAstrologer = async (
  ...
  try {
    ...
    const customer_img = callRecord.customer?.customer_img || "";
    const imgPath = path.join(
      process.cwd(),
      "public",
      constants.customer_image_path
    ...
    const customer_img_url =
      customer_img && fs.existsSync(fullImgPath)
        ? `${constants.base_url}${constants.customer_image_path}${customer_img}`
    ...
    console.error("Error in inProgressIntakeDetailForAstrologer:", err);
    return {};
  }
};
...
export const getIsChatAllowed = async (channel_name = null) => {
  let is_chat_allowed = 0;
  let uniqeid = "";
  let customer_uni_id = "";
  let astrologer_uni_id = "";

  const message_allowed_for_astrologer_after_chat_end = getConfig(
    "message_allowed_for_astrologer_after_chat_end"
  );

  if (
    channel_name &&
    message_allowed_for_astrologer_after_chat_end &&
    !isNaN(message_allowed_for_astrologer_after_chat_end) &&
    Number(message_allowed_for_astrologer_after_chat_end) > 0
  ) {
    ...
    if (chat_channel_history) {
      uniqeid = chat_channel_history.uniqeid;

      const timeLimit = new Date(
        Date.now() - message_allowed_for_astrologer_after_chat_end * 60 * 1000
      );
      ...
      if (callHistory) {
        customer_uni_id = callHistory.customer_uni_id;
        astrologer_uni_id = callHistory.astrologer_uni_id;
        is_chat_allowed = 1;
      }
    }
...
export const declineCallRequest = async (uniqeid = '', status = '') => {
  let result = {};
  ...
  if (calls) {
    ...
    await removeBusyStatus(calls.astrologer_uni_id);
    ...
    if (status === 'Declined(Customer)' || status === 'Declined(User)') {
      const astroNotify = {
        title: customer?.name || "",
        description: 'Voice Call Declined by Customer',
        chunk: [astrologer?.user?.user_fcm_token],
        type: 'cancel',
        channelName: getConfig('company_name'),
    ...
    if (status === 'Declined(Astrologer)' || status === 'Declined(User)') {
      const customerNotify = {
        title: astrologer?.display_name || "",
        description: 'Voice Call Declined by Astrologer',
        chunk: [customer?.user_fcm_token],
        type: 'cancel',
        channelName: getConfig('company_name'),
        user_uni_id: calls.customer_uni_id,
        ...
        duration: 0,
        start_time: getConfig('current_datetime'),
        click_action: route("waitingTime"),
        notification_id: calls.id,
        cancel_status: 1,
      };
      await sendNotification(customerNotify);
...
export const startAssistantChat = async (req, call_type) => {
  try {
    const customer = await getCustomerById(req.user_uni_id);
    const astrologer = await Astrologer.findOne({
      where: { astrologer_uni_id: req.astrologer_uni_id },
      include: [
        {
          model: User,
          as: "user",
          required: true,
          where: { user_uni_id: req.astrologer_uni_id }
        }
      ]
    });
    ...
    if (customer) {
      if (astrologer) {
        const callHistory = await CallHistory.findOne({
          where: {
            astrologer_uni_id: req.astrologer_uni_id,
            customer_uni_id: req.user_uni_id,
            status: "completed",
            offer_type: 0
          }
        });
        ...
        if (callHistory) {
          const senddata = {
            uniqeid: callHistory.uniqeid,
            name: astrologer.user?.name || astrologer.name // from joined user table
          };
          ...
          result = {
            ...
            join_status: 1,
            data: senddata,
            msg: "You are connected to the assistant chat"
          };
        } else {
          result = {
            status: 0,
            join_status: 0,
            msg: "You can chat with astrologers assistant only when you have taken a paid session with the astrologer"
          };
        }
      } else {
        result = {
    ...
    return {
      status: 0,
      msg: "An error occurred while starting the assistant chat.",
...
export const getAstroDataForCustomer = async (user_uni_id, is_first = 0) => {
  const customer_uni_id = user_uni_id;
  const currency_detail = await getCurrency(customer_uni_id, 'all');
  const currency_code = currency_detail.currency_code;
  const currency_symbol = currency_detail.currency_symbol;
  const exchange_rate = currency_detail.exchange_rate;
  const isFirstCall = await isFirstUser(customer_uni_id);
  const first_call_offer = config('first_call_offer');
  const first_call_price = config('first_call_price_per_min');
  const first_call_free_minutes = config('first_call_free_minutes');

};


export const getVoiceCallRequest = async (filter_array) => {
  try {
    const whereConditions = {
      call_type: "call",
      [Op.or]: [{ status: "request" }, { status: "in-progress" }],
    ...
    for (const val of res) {
      ...
      //   "datetime"
      // );
      val.dataValues.order_datetime = val.dataValues.created_at;
      val.dataValues.max_duration = parseInt(val.waiting_time, 10);
      val.dataValues.cust_balance = await getTotalBalanceById(
        val.customer_uni_id
      );
...
export const saveIntake = async (attributes) => {
  try {
    const checkAlreadyExist = await Intake.findOne({
      where: { uniqeid: attributes.uniqeid }
    });

    if (!checkAlreadyExist) {
      const { api_key, ...cleanedAttributes } = attributes;
      cleanedAttributes.status = 1;

      const created = await Intake.create(cleanedAttributes);
    ...
    console.error("Error in saveIntake:", err);
    return {
      status: 0,
...
export const cronRefreshCall = async () => {
  try {
    const realtime_database = await firebaseRealtimeDbConnection();

    const ivr_count = constants.ivr_count;
    const chat_in_progress_waiting = constants.chat_in_progress_waiting;
    const chat_in_progress_customer_rejoin_waiting = constants.chat_in_progress_customer_rejoin_waiting;
    const chat_in_progress_astrologer_rejoin_waiting = constants.chat_in_progress_astrologer_rejoin_waiting;
    const chat_in_progress_no_response_waiting = constants.chat_in_progress_no_response_waiting;

    const video_call_in_progress_customer_rejoin_waiting = constants.chat_in_progress_customer_rejoin_waiting;
    const video_call_in_progress_astrologer_rejoin_waiting = constants.chat_in_progress_astrologer_rejoin_waiting;
    const video_call_in_progress_no_response_waiting = constants.chat_in_progress_no_response_waiting;

    const voice_call_in_progress_customer_rejoin_waiting = constants.chat_in_progress_customer_rejoin_waiting;
    const voice_call_in_progress_astrologer_rejoin_waiting = constants.chat_in_progress_astrologer_rejoin_waiting;
    const voice_call_in_progress_no_response_waiting = constants.chat_in_progress_no_response_waiting;

    const result = [];

    const thismodel = await CallHistory.findAll({
      where: {
        [Op.or]: [
          { status: 'queue' },
          { status: 'queue_request' },
    ...
    for (const value of thismodel) {
      let is_virtual_astrologer = 0;

      if (value.call_type === 'chat' && ['queue_request', 'request', 'in-progress'].includes(value.status)) {
        const check_virtual_astrologer = await Astrologer.findOne({
          where: {
        ...
        if (check_virtual_astrologer) {
          is_virtual_astrologer = check_virtual_astrologer.is_virtual;
        }
      }

      const in_app_voice_call = value.is_inapp_voice_call;
      const customer_offline_at = value.customer_offline_at;
      const astrologer_offline_at = value.astrologer_offline_at;
      const ivr_start_from = value.ivr_start_from;
      const waiting_for_request = value.waiting_for_request;
      const in_progress_waiting = new Date(new Date(waiting_for_request).getTime() + (value.duration * 1000));

      // Handle queue_request status
      if (['queue_request'].includes(value.status)) {
        if (value.call_type === 'chat') {
          if (ivr_start_from && value.ivr_count && ivr_start_from < getConfig('current_datetime') && value.ivr_count > 0 && value.call_type === 'chat') {
            const user = await User.findOne({ where: { user_uni_id: value.customer_uni_id } });
            const astroWhite = await exotelCustomerWhitelistCurl(user.phone);
            const astroWhite2 = await exotelIncomingCallRequestCurl(user.phone);
            const new_ivr_count = value.ivr_count - 1;

            if (new_ivr_count == 0) {
              await value.update({ ivr_start_from: null });
            }
            await value.update({ ivr_count: new_ivr_count });
          } else if (waiting_for_request && waiting_for_request < getConfig('current_datetime')) {
            await value.update({ status: 'Customer Not Receive Chat' });

            if (realtime_database && realtime_database !== false) {
              const chat_channel_name = `CHAT/${value.customer_uni_id}-${value.astrologer_uni_id}`;
              const updateData = {
                chatEnd: true,
                chatAccepted: false,
              };
              await realtime_database.ref(chat_channel_name).update(updateData);
            }

            // Send notifications
            const customer = await getCustomerById(value.customer_uni_id);
            const astrologer = await Astrologer.findOne({
              include: [{
                model: User,
                where: { user_uni_id: value.astrologer_uni_id }
              }]
            });

            if (customer && astrologer) {
              const astroNotify = {
                title: customer.name,
                description: 'Chat not accepted by you',
                chunk: [astrologer.user.user_fcm_token],
                type: 'android',
                channelName: getConfig('company_name'),
                user_uni_id: value.customer_uni_id,
                astrologer_uni_id: value.astrologer_uni_id,
                duration: 0,
                start_time: getConfig('current_datetime'),
              ...
              const customerNotify = {
                title: astrologer.display_name,
                description: 'Astrologer not receive your chat',
                chunk: [customer.user_fcm_token],
                type: 'cancel',
                channelName: getConfig('company_name'),
                user_uni_id: value.customer_uni_id,
            ...
            await removeBusyStatus(value.astrologer_uni_id);
            const waitingCustomerData = await waitingCustomer(value.astrologer_uni_id);
            if (waitingCustomerData?.call_type) {
              await startCall(waitingCustomerData, waitingCustomerData.call_type);
            }
          }
        } else if (value.call_type === 'video') {
          if (ivr_start_from && value.ivr_count && ivr_start_from < getConfig('current_datetime') && value.ivr_count > 0 && value.call_type === 'video') {
            const user = await User.findOne({ where: { user_uni_id: value.customer_uni_id } });
            const astroWhite = await exotelCustomerWhitelistCurl(user.phone);
            const astroWhite2 = await exotelIncomingCallRequestCurl(user.phone, 'video');
            const new_ivr_count = value.ivr_count - 1;
            ...
            await value.update({ status: 'Customer Not Receive Video Call' });

            if (realtime_database && realtime_database !== false) {
              const chat_channel_name = `CHAT/${value.customer_uni_id}-${value.astrologer_uni_id}-Video`;
              const updateData = {
                chatEnd: true,
                chatAccepted: false,
              };
            ...
            if (customer && astrologer) {
              const astroNotify = {
                title: customer.name,
                description: 'Video call not accepted by you',
                chunk: [astrologer.user.user_fcm_token],
                type: 'android',
                channelName: getConfig('company_name'),
                user_uni_id: value.customer_uni_id,
              ...
              const customerNotify = {
                title: astrologer.display_name,
                description: 'Astrologer not receive your video call',
                chunk: [customer.user_fcm_token],
                type: 'cancel',
                channelName: getConfig('company_name'),
                user_uni_id: value.customer_uni_id,
        ...
        } else if (value.call_type === 'call') {
          if (in_app_voice_call && in_app_voice_call === 1) {
            if (ivr_start_from && value.ivr_count && ivr_start_from < getConfig('current_datetime') && value.ivr_count > 0 && value.call_type === 'call') {
              const user = await User.findOne({ where: { user_uni_id: value.customer_uni_id } });
              const astroWhite = await exotelCustomerWhitelistCurl(user.phone);
              const astroWhite2 = await exotelIncomingCallRequestCurl(user.phone, 'call', in_app_voice_call);
              const new_ivr_count = value.ivr_count - 1;
              ...
              await value.update({ status: 'Customer Not Receive Voice Call' });

              if (realtime_database && realtime_database !== false) {
                const chat_channel_name = `CHAT/${value.customer_uni_id}-${value.astrologer_uni_id}-Call`;
                const updateData = {
                  chatEnd: true,
                  chatAccepted: false,
                };
              ...
              if (customer && astrologer) {
                const astroNotify = {
                  title: customer.name,
                  description: 'Voice call not accepted by you',
                  chunk: [astrologer.user.user_fcm_token],
                  type: 'android',
                  channelName: getConfig('company_name'),
                  user_uni_id: value.customer_uni_id,
                ...
                const customerNotify = {
                  title: astrologer.display_name,
                  description: 'Astrologer not receive your voice call',
                  chunk: [customer.user_fcm_token],
                  type: 'cancel',
                  channelName: getConfig('company_name'),
                  user_uni_id: value.customer_uni_id,
      ...
      } else if (['request'].includes(value.status)) {
        if (value.call_type === 'chat') {
          if (ivr_start_from && value.ivr_count && ivr_start_from < getConfig('current_datetime') && value.ivr_count > 0 && value.call_type === 'chat') {
            if (is_virtual_astrologer === 1) {
              const virtualReqUpdateData = {
              ...
              await CallHistory.update(virtualReqUpdateData, { where: { uniqeid: value.uniqeid } });

              if (realtime_database && realtime_database !== false) {
                const chat_channel_name = `CHAT/${value.customer_uni_id}-${value.astrologer_uni_id}`;
                const updateData = {
                  chatAccepted: true,
                  astrologer_status: true,
                };
                await realtime_database.ref(chat_channel_name).update(updateData);
              }

              const chnnl_name = `CHAT/${value.customer_uni_id}-${value.astrologer_uni_id}`;
              const virtualMessageData = {
                user_uni_id: value.astrologer_uni_id,
                uniqeid: value.uniqeid,
                channel_name: chnnl_name,
                message: getConfig('virtual_astrologer_first_message'),
                message_type: 'Text',
                call_type: 'chat',
                is_first_chat: 1,
                is_assistant_chat: 0,
                is_virtual_astrologer: is_virtual_astrologer,
                is_virtual_astrologer_first_msg: 1
              };
              await saveChat(virtualMessageData);
            } else {
              const user = await User.findOne({ where: { user_uni_id: value.astrologer_uni_id } });
              const astroWhite = await exotelCustomerWhitelistCurl(user.phone);
              const astroWhite2 = await exotelIncomingCallRequestCurl(user.phone);
              const new_ivr_count = value.ivr_count - 1;
            ...
            await value.update({ status: 'Astrologer Not Receive Chat' });
            await checkCallNotResponseCount(value.uniqeid);
          ...
          if (ivr_start_from && value.ivr_count && ivr_start_from < getConfig('current_datetime') && value.ivr_count > 0 && value.call_type === 'video') {
            ...
            await value.update({ status: 'Astrologer Not Receive Video Call' });
            await checkCallNotResponseCount(value.uniqeid);
          ...
          if (in_app_voice_call && in_app_voice_call === 1) {
            if (ivr_start_from && value.ivr_count && ivr_start_from < getConfig('current_datetime') && value.ivr_count > 0 && value.call_type === 'call') {
              ...
              await value.update({ status: 'Astrologer Not Receive Voice Call' });
              await checkCallNotResponseCount(value.uniqeid);
        ...
        if (value.call_type === 'chat') {
          const channel_name = `CHAT/${value.customer_uni_id}-${value.astrologer_uni_id}`;
          const last_msg_datetime = await getLastChatMsgTime(channel_name);
          let is_time_not_remaining = 0;

          let chat_end_flag = 0;
          if (value.call_start && value.call_start !== null) {
            const current_time = Math.floor(Date.now() / 1000);
            const chat_start_time = Math.floor(new Date(value.call_start).getTime() / 1000);
            const time_difference = current_time - chat_start_time;
            const total_time_in_second = value.waiting_time;
            const remaining_time_in_second = total_time_in_second - time_difference;

            if (remaining_time_in_second > 0) {
              if (customer_offline_at && customer_offline_at !== null) {
                const customer_rejoin_till = new Date(new Date(customer_offline_at).getTime() + (chat_in_progress_customer_rejoin_waiting * 1000));

                if (customer_rejoin_till < getConfig('current_datetime')) {
                  chat_end_flag = 1;
                }
              } else if (astrologer_offline_at && astrologer_offline_at !== null) {
                const astrologer_rejoin_till = new Date(new Date(astrologer_offline_at).getTime() + (chat_in_progress_astrologer_rejoin_waiting * 1000));

                if (astrologer_rejoin_till < getConfig('current_datetime')) {
                  chat_end_flag = 1;
                }
              } else {
                if (last_msg_datetime && last_msg_datetime !== null) {
                  const no_response_till = new Date(new Date(last_msg_datetime).getTime() + (chat_in_progress_no_response_waiting * 1000));

                  if (no_response_till < getConfig('current_datetime')) {
                    chat_end_flag = 1;
                  }
                }
              }
            } else {
              is_time_not_remaining = 1;
              chat_end_flag = 1;
            }
          }
          ...
          if (chat_end_flag === 1) {
            ...
            if (is_time_not_remaining === 1) {
              duration = value.waiting_time;
              call_end = new Date(new Date(value.call_start).getTime() + (duration * 1000));
            } else if (last_msg_datetime && last_msg_datetime !== null) {
              call_end = last_msg_datetime;
              duration = Math.floor((new Date(call_end).getTime() - new Date(value.call_start).getTime()) / 1000);
            } else {
              call_end = new Date();
              duration = Math.floor((new Date(call_end).getTime() - new Date(value.call_start).getTime()) / 1000);
              if (duration > value.waiting_time) {
                duration = value.waiting_time;
                call_end = new Date(new Date(value.call_start).getTime() + (duration * 1000));
              }
            }

            if (call_end && duration > 0) {
              const sendData = {
                uniqeid: value.uniqeid,
                startTime: value.call_start,
                endTime: call_end,
                duration: duration,
                call_type: value.call_type,
              };
              await callTransations(sendData);
            }
          ...
          let video_call_end_flag = 0;
          let video_call_end_flag_on_stuck = 0;

          if (value.call_start && value.call_start !== null) {
            const current_time = Math.floor(Date.now() / 1000);
            const video_call_start_time = Math.floor(new Date(value.call_start).getTime() / 1000);
            ...
            if (remaining_time_in_second > 0) {
              if (customer_offline_at && customer_offline_at !== null) {
                const customer_rejoin_till = new Date(new Date(customer_offline_at).getTime() + (video_call_in_progress_customer_rejoin_waiting * 1000));

                if (customer_rejoin_till < getConfig('current_datetime')) {
                  video_call_end_flag = 1;
                }
              }

              if (astrologer_offline_at && astrologer_offline_at !== null) {
                const astrologer_rejoin_till = new Date(new Date(astrologer_offline_at).getTime() + (video_call_in_progress_astrologer_rejoin_waiting * 1000));
              ...
              if (remaining_time_in_second < -300) {
                video_call_end_flag_on_stuck = 1;
              } else {
                video_call_end_flag = 1;
              }
            }
          ...
          if (video_call_end_flag_on_stuck === 1) {
            await value.update({ status: 'Declined(Astrologer Offline)' });
            await removeBusyStatus(value.astrologer_uni_id);
          }

          if (video_call_end_flag === 1) {
            if (realtime_database && realtime_database !== false) {
              const chat_channel_name = `CHAT/${value.customer_uni_id}-${value.astrologer_uni_id}-Video`;
              const updateData = {
                chatEnd: true,
          ...
          if (in_app_voice_call && in_app_voice_call === 1) {
            let voice_call_end_flag = 0;
            let voice_call_end_flag_on_stuck = 0;

            if (value.call_start && value.call_start !== null) {
              const current_time = Math.floor(Date.now() / 1000);
              const voice_call_start_time = Math.floor(new Date(value.call_start).getTime() / 1000);
              const time_difference = current_time - voice_call_start_time;
              const total_time_in_second = value.waiting_time;
              const remaining_time_in_second = total_time_in_second - time_difference;

              if (remaining_time_in_second > 0) {
                if (customer_offline_at && customer_offline_at !== null) {
                  const customer_rejoin_till = new Date(new Date(customer_offline_at).getTime() + (voice_call_in_progress_customer_rejoin_waiting * 1000));

                  if (customer_rejoin_till < getConfig('current_datetime')) {
                    voice_call_end_flag = 1;
                  }
                }

                if (astrologer_offline_at && astrologer_offline_at !== null) {
                  const astrologer_rejoin_till = new Date(new Date(astrologer_offline_at).getTime() + (voice_call_in_progress_astrologer_rejoin_waiting * 1000));
                ...
                if (remaining_time_in_second < -300) {
                  voice_call_end_flag_on_stuck = 1;
                } else {
                  voice_call_end_flag = 1;
                }
              }
            ...
            if (voice_call_end_flag_on_stuck === 1) {
              await value.update({ status: 'Declined(Astrologer Offline)' });
              await removeBusyStatus(value.astrologer_uni_id);
            }

            if (voice_call_end_flag === 1) {
              if (realtime_database && realtime_database !== false) {
                const chat_channel_name = `CHAT/${value.customer_uni_id}-${value.astrologer_uni_id}-Call`;
                const updateData = {
                  chatEnd: true,
        ...
        const minusOneMinute = new Date(Date.now() - 60 * 1000);
        if (value.created_at < minusOneMinute) {
          const waitingCustomerData = await waitingCustomer(value.astrologer_uni_id);
          if (waitingCustomerData?.call_type) {
            waitingCustomerData.call_by_cronjob = 1;
            await startCall(waitingCustomerData, waitingCustomerData.call_type);
          }
        }
      }
...
export const getLastChatMsgTime = async (channel_name = null) => {
  try {
    let last_msg_datetime = '';
    ...
    if (channel_name && channel_name !== null) {
      ...
      if (chat_channel_history) {
        last_msg_datetime = chat_channel_history.created_at;
      }
    }

    return last_msg_datetime;
  } catch (error) {
    console.error('Error in getLastChatMsgTime:', error);
    return '';
  }
};

export const checkCallNotResponseCount = async (uniqeid = '') => {
  try {
    const result = {};
    ...
    if (uniqeid) {
      ...
      if (callHistory) {
        const astrologer_uni_id = callHistory.astrologer_uni_id;

        // Increment no_response_count
        await Astrologer.increment('no_response_count', {
          where: { astrologer_uni_id }
        });

        // Check if no_response_count > 1
        const astrologer = await Astrologer.findOne({
          where: {
            astrologer_uni_id,
            no_response_count: {
        ...
        if (astrologer) {
          ...
          await astrologer.update(astrodatetime);

          // Remove queue list
          await removeQueueList(astrologer_uni_id, 'Declined(Astrologer Offline)');

          // Update user activity for each status
          for (const value of Object.keys(astrodatetime)) {
            await userActivityUpdate(value, astrologer_uni_id);
          }
        }
      }
    }
    ...
    console.error('Error in checkCallNotResponseCount:', error);
    return {};
  }
};